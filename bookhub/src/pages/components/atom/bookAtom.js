...
export default function Book(props) {
    ...
    return (
        <div ref={inputRef} className={"bookContainer"} onMouseEnter={() => { hoverOn() }} onMouseLeave={() => { hoverOff() }}>
            ...
            {hover ? <HoverAtom cb ={props.cb} style={{ top: `${metrics.top + metrics.height / 2}px`, left: `${metrics.left + metrics.width / 2}px`}} description={props.description} name={props.name} authors = {props.authors}></HoverAtom> : <></>}
            {/* <button onClick={() => {console.log(props)}}>Add to Book Shelf</button> */}
        </div>
